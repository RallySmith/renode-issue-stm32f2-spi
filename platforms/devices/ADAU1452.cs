// Simple model to provide Analog Devices ADAU1452 basic SPI support
//
...
//
// Based on ADAU1452 datasheet Rev.D
using System;
using System.IO;
using System.Text;
...
{
    public class ADAU1452 : ISPIPeripheral, IGPIOReceiver, IProvidesRegisterCollection<WordRegisterCollection>
    {
        public ADAU1452()
        {
            this.memDM0 = new uint[20480];
            this.memDM1 = new uint[20480];
            this.memProgram = new uint[8192];
        ...
        public void Reset()
        {
            // As per ADAU1452 datasheet RevD the state of the RAM is
        ...
        private WordRegisterCollection CreateRegisters()
        {
            ...
            // actual ADAU1452 hardware, but just provides "storage"
            ...
            registersMap = new Dictionary<long, WordRegister>
            {
                ...
                { (long)Registers.SOFTWARE_VALUE_0, new WordRegister(this, 0x0000) // Software Panic Value 0
                  .WithValueField(0, 16, name: "UNIMPLEMENTED")
                },
                { (long)Registers.SOFTWARE_VALUE_1, new WordRegister(this, 0x0000) // Software Panic Value 1
                  .WithValueField(0, 16, name: "UNIMPLEMENTED")
                },
                ...
                { (long)Registers.ASRC7_RATIO, new WordRegister(this, 0x0000) // ASRC ratio (ASRC 7, Channel 14 and Channel 15)
                  .WithValueField(0, 16, FieldMode.Read, name: "UNIMPLEMENTED_RO")
                },
        ...
        private (uint[], ushort) MemoryIndex(ushort address)
        {
            ushort index = 0;
            uint[] memblk = null;
            if (address < 0x5000)
        ...
        public uint ReadMemory(ushort address)
        {
            uint value = 0x00000000;
            var maccess = MemoryIndex(address);
        ...
        public void WriteMemory(ushort address, uint value)
        {
            ...
            // As per the ADAU1452 datasheet Rev.D Table 62 we mimic
            // the "Software Safeload" approach. We trigger the
            // operation on the 0x6006 num_SafeLoad write.
            if ((0x6006 == address) && (value >= 1))
            {
                ushort safeAddress = (ushort)memDM1[5]; // 0x6005
                ...
                if (null != safeAccess.Item1)
                {
                    ...
                    this.Log(LogLevel.Debug, "WRITE Safeload: address 0x{0:X}", safeAddress);
                    for (var dIndex = 0; (dIndex < value); dIndex++)
                    {
                        uint safeValue = memDM1[dIndex];
                        parameterAddress[parameterIndex + dIndex] = safeValue;
                        this.Log(LogLevel.Debug, "WRITE Safeload: 0x{0:X} = 0x{1:X}", (safeAddress + dIndex), safeValue);
                    }
                }
            }
        }

        public void SaveMemory(WriteFilePath fileName, ushort address, ushort numWords)
        {
            this.Log(LogLevel.Debug, "SaveMemory address 0x{0:X} numWords {1}", address, numWords);
            try
            {
        ...
        private uint[] memDM1;
        // 0xC000 -> 0xDFFF      8192 words     Program Memory          32-bits
        private uint[] memProgram;
        ...
        private enum Registers : ushort
        {
            ...
            SOFTWARE_VALUE_0 = 0xF433,
            SOFTWARE_VALUE_1 = 0xF434,
